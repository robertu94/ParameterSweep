...
class Distribution
  : public RandomNumberGenerator<typename RandomNumberDistribution::result_type>
{
...
public:
  ...
  Distribution(RandomNumberEngine gen, RandomNumberDistribution dist)
    : dist(dist)
    , gen(gen)
  {}

  typename RandomNumberDistribution::result_type operator()() override
  {
    return dist(gen);
  }
  void seed(typename RandomNumberDistribution::result_type value) override
  {
    return gen.seed(value);
  }
...
template <class RandomNumberEngine, class RandomNumberDistribution>
auto
make_dist(RandomNumberEngine gen, RandomNumberDistribution dist)
{
  return std::make_unique<
    Distribution<RandomNumberEngine, RandomNumberDistribution>>(gen, dist);
}
